...
def test_add_stackline(session, request_1):
    assert (
        session.query(StackLine).filter(StackLine.request_id == request_1.id).one_or_none() is None
    )
    add_stack_line(
        session, request_id=request_1.id, position=0, indent=1, duration=1, code_line="code"
    )
    session.commit()
