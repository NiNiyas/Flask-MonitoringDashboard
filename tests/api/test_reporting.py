...
@pytest.mark.parametrize(
    "request_2__time_requested", [datetime.utcnow() - timedelta(days=1, hours=6)]
)
@pytest.mark.parametrize("request_2__duration", [100])
@pytest.mark.skipif(
    sys.version_info < (3,), reason="For some reason, this doesn't work in python 2.7."
)
def test_make_report_post_not_significant(dashboard_user, endpoint, request_1, request_2, session):
    ...
    assert question1["latencies_samples"] == {
        "baseline": [request_2.duration],
        "comparison": [request_1.duration],
    }
