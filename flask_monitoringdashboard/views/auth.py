import flask
from flask import redirect, render_template, url_for, request, jsonify, flash
from sqlalchemy.exc import IntegrityError
from sqlalchemy.orm.exc import NoResultFound

from flask_monitoringdashboard import blueprint, config
from flask_monitoringdashboard.core.auth import (
    on_logout,
    on_login,
    secure,
    is_admin,
...
def login():
    ...
    if request.method == "POST":
        ...
        if user is not None:
            ...
            flash("Error logging in. Please check your credentials.", "error")
...
def user_edit():
    ...
    if flask.session.get(config.link + "_user_id") == user_id and not is_admin:
        return (
            jsonify({"message": "Cannot remove the admin permissions from itself."}),
            BAD_REQUEST_STATUS,
        )
        ...
        try:
            ...
            if old_password is not None:
                if user.check_password(old_password):
                    ...
                    return (
                        jsonify({"message": "Old password doesn't match."}),
                        BAD_REQUEST_STATUS,
                    )
