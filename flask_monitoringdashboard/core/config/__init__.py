...
class Config(object):
    ...
    def __init__(self):
        ...
        self.disable_csrf_protection = False
    ...
    def init_from(self, file=None, envvar=None, log_verbose=False):
        ...
        - DISABLE_CSRF_PROTECTION: Boolean if you want to use the dashboard with Flask-WTF's CSRF protection.
        Default value is False
        ...
        try:
            ...
            self.blueprint_name = parse_string(
                parser, "dashboard", "BLUEPRINT_NAME", self.blueprint_name
            )
            ...
            self.outlier_detection_constant = parse_literal(
                parser,
                "dashboard",
                "OUTlIER_DETECTION_CONSTANT",
                self.outlier_detection_constant,
            )
            self.sampling_period = (
                parse_literal(
                    parser, "dashboard", "SAMPLING_RATE", self.sampling_period
                )
                / 1000.0
            )
            ...
            self.disable_csrf_protection = parse_bool(
                parser,
                "dashboard",
                "DISABLE_CSRF_PROTECTION",
                self.disable_csrf_protection,
            )
            self.brand_name = parse_string(
                parser, "dashboard", "BRAND_NAME", self.brand_name
            )
            self.title_name = parse_string(
                parser, "dashboard", "TITLE_NAME", self.title_name
            )
            self.description = parse_string(
                parser, "dashboard", "DESCRIPTION", self.description
            )
            self.show_login_banner = parse_bool(
                parser, "dashboard", "SHOW_LOGIN_BANNER", self.show_login_banner
            )
            self.show_login_footer = parse_bool(
                parser, "dashboard", "SHOW_LOGIN_FOOTER", self.show_login_footer
            )

            # parse 'authentication'
            self.username = parse_string(
                parser, "authentication", "USERNAME", self.username
            )
            self.password = parse_string(
                parser, "authentication", "PASSWORD", self.password
            )
            ...
            # database
            self.database_name = parse_string(
                parser, "database", "DATABASE", self.database_name
            )
            self.table_prefix = parse_string(
                parser, "database", "TABLE_PREFIX", self.table_prefix
            )
